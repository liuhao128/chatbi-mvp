"""Prompt 构造模块负责将 Schema 信息、Few-shot 示例和用户问题组装为完整 Prompt。
Schema 和示例在此集中维护，便于后续课程中动态扩展。
第7课增强：新增 RULES（业务规则注入层）与 ERROR_GUARDS（错误防护层），通过 build_prompt 的可选参数控制是否注入，实现 Prompt 策略的灵活切换。"""

# ==================== Schema 定义 ====================
SCHEMA = """表：dim_customers（客户维度表）
- customer_id INT 主键
...
示例2：
问题：按客户类型统计订单数量
SQL：SELECT c.customer_type, COUNT(*) AS order_count 
     FROM sales_orders o 
...
示例3：
问题：查询2026年第一季度的总费用
SQL：SELECT SUM(rd_expense + selling_expense + admin_expense + finance_expense) AS total_expense 
     FROM finance_expenses 
     WHERE expense_date >= '2026-01-01' AND expense_date < '2026-04-01';"""
...
3. 订单统计范围：统计收入、订单量、客单价等指标时，必须过滤order_status = 'completed'，排除cancelled 和pending
4. 汇率转换：涉及多币种收入汇总时，必须通过 order_date 和currency 关联 exchange_rates 表，使用 rate_to_cny 折算为人民币
5. 时间范围："最近N个月"使用 DATE_SUB(CURDATE(), INTERVAL N MONTH) 作为起始边界；"本月"指当月1日至当前日期
6. 费用层级：selling_expense 是销售费用总项，包含marketing_expense、logistics_expense、warranty_expense，汇总时不得重复计算
...
- 过滤遗漏：所有收入类统计必须包含WHERE order_status = 'completed'
...
def build_prompt(
    ...
    """
    system_msg = "你是一个专业的 SQL 生成助手，擅长根据业务问题生成标准MySQL 查询语句。"
    if use_rules or use_guards:
        system_msg = "你是一个专业的 SQL 生成助手，擅长根据业务问题生成标准MySQL 查询语句。请严格遵守给定的业务规则，避免常见错误。"
    
    prompt = f"""【数据库Schema】{SCHEMA}"""
